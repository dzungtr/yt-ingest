...
Ingest a list of YouTube URLs, extract structured notes via the Anthropic API, and build a locally queryable knowledge base. Optimised for a single user processing ~20 videos at a time on a workstation with an NVIDIA GPU available for fallback transcription. The deliverable is a CLI tool — no web UI, no multi-user, no cloud deploy.
...
flowchart TD
    ...
    J --> K[Normalize + chunk<br/><b>anthropic</b> token counter<br/>target 80k tok/chunk]
    K --> L[Cache to disk<br/>./transcripts/video_id.json]

    L --> M[Extract structured notes<br/><b>anthropic</b> SDK<br/>model: claude-sonnet-4-6<br/>response_format: JSON]
    ...
    T --> U[Aggregate prompt<br/><b>anthropic</b> SDK<br/>model: claude-opus-4-7<br/>200k ctx window]
    U --> V[Write ./notes/_index.md<br/>themes, contradictions, gaps]

    V --> W[CLI query mode<br/><b>typer</b><br/>→ embed query → FAISS topK<br/>→ Claude answer w/ citations]
```
...
| LLM extraction | `anthropic` (official SDK) | — |
...
|---|---|---|
| `ANTHROPIC_API_KEY` | ✅ Yes | Extraction + synthesis |
...
│   │   ├── ytdlp.py
│   │   └── whisper.py
│   ├── extract.py               # Anthropic structured extraction
...
- [ ] All Anthropic calls go through one wrapper that handles retries (exponential backoff, max 3) and logs token usage
- [ ] `.env.example` exists; real `.env` is in `.gitignore`
- [ ] `pytest` passes with no real network calls (mock Anthropic, mock youtube-transcript-api)
...
- **Anthropic model selection.** Use `claude-sonnet-4-6` for per-video extraction (cheaper, plenty capable for structured output). Use `claude-opus-4-7` only for the synthesis pass where cross-document reasoning matters.
- **Token accounting.** Wrap every Anthropic call to log `input_tokens`, `output_tokens`, and a running total. At the end of a `run`, print a cost estimate to stderr.
- **Determinism in tests.** All fixtures live in `tests/fixtures/`. Never call out to YouTube or Anthropic in tests. Use `pytest-mock` to patch the SDK clients.
...
7. Anthropic wrapper (retries, token logging)
